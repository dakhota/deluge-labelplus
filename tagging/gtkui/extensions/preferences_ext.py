...
from tagging.common.literals import (
  TITLE_USER_INTERFACE, TITLE_DAEMON, TITLE_TAG_DEFAULTS,
  TITLE_SELECT_FOLDER,
...
GLADE_FILE = tagging.common.get_resource("blk_preferences.glade")
...
class PreferencesExt(WidgetEncapsulator):
  ...
  def _setup_widgets(self):
    ...
    self._lbl_defaults.set_markup("<b>%s</b>" % _(TITLE_TAG_DEFAULTS))
  ...
  def _setup_test_combo_box(self):
    ...
    prop_store = tagging.gtkui.common.gtklib.liststore_create(str,
      [_(x) for x in tagging.common.config.autotag.PROPS])
  ...
  def _index_widgets(self):
    ...
    self._ui_option_group = (
      ...
      self._chk_filter_include_subtags,
      self._chk_status_bar,
      self._chk_status_bar_include_subtags,
    )
    ...
    self._dependency_widgets = {
      self._chk_status_bar: (self._chk_status_bar_include_subtags,),
    }
  ...
  def _request_prefs(self):
    ...
    def on_timeout():
      ...
      if self.valid:
        ...
        self._report_error(STR_LOAD_PREFS, TaggingError(ERR_TIMED_OUT))
    ...
    deferred = client.tagging.get_preferences()
  ...
  def _save_prefs(self):
    ...
    def on_timeout(prefs):
      ...
      if self.valid:
        self._sw_settings.set_sensitive(True)
        self._report_error(STR_SAVE_PREFS, TaggingError(ERR_TIMED_OUT))
    ...
    same = tagging.common.dict_equals(prefs, self._prefs) and \
      tagging.common.dict_equals(options, self._plugin.config["common"])

    if not same:
      deferred = client.tagging.set_preferences(prefs)
  ...
  def _load_prefs(self, prefs):
    ...
    for group in self._option_groups:
      self._set_widget_values(group, prefs["tag"])


  def _get_prefs(self):

    prefs = copy.deepcopy(tagging.common.config.CONFIG_DEFAULTS["prefs"])

    self._get_widget_values(self._daemon_option_group, prefs["options"])

    for group in self._option_groups:
      self._get_widget_values(group, prefs["tag"])
  ...
  def _do_revert_to_defaults(self, widget):
    ...
    self._load_prefs(tagging.common.config.CONFIG_DEFAULTS["prefs"])
    self._load_ui_options(tagging.gtkui.config.CONFIG_DEFAULTS["common"])
  ...
  def _do_test_criteria(self, *args):
    ...
    prop = tagging.common.config.autotag.PROPS[index]
