...
STATUS_ICON_FILE = tagging.common.get_resource("tagging_icon.png")
...
class StatusBarExt(object):
  ...
  def _update_loop(self):
    ...
    ids = ext.get_selected_tags() if ext else None
    if not ids or (len(ids) == 1 and ID_ALL in ids):
      ext = self._plugin.get_extension("TorrentViewExt")
      ids = ext.get_selected_torrent_tags() if ext else None
    ...
    if (ids and self._plugin.config["common"]["status_bar"] and
      ...
      include = self._plugin.config["common"]["status_bar_include_subtags"]
      if include:
        if ids[0] != ID_NONE:
          tooltip += "/*"
      ...
      deferred = client.tagging.get_tag_bandwidth_usages(list(set(ids)))
