...
class TagStore(object):
  ...
  def get_descendent_ids(self, id_, max_depth=-1):
    ...
    def add_descendents(iter_, depth):
      ...
      children = tagging.gtkui.common.gtklib.treemodel_get_children(
        self.model, iter_)

      for child in children:
        descendents.append(self.model[child][self.TAG_ID])
  ...
  def user_tags(self, ids):

    if not ids:
      return False

    for id_ in ids:
      if not self.is_user_tag(id_):
        return False
  ...
  def _build_fullname_index(self, data):
    ...
    def resolve_fullname(id_):
      ...
      while id_:
        ...
        id_ = tagging.common.tag.get_parent_id(id_)
  ...
  def _build_store(self, data):
    ...
    for id_ in sorted(data):
      if id_ in RESERVED_IDS:
        ...
        parent_id = tagging.common.tag.get_parent_id(id_)
    ...
    sorted_model.set_sort_func(self.TAG_DATA, data_sort_asc)
    sorted_model.set_sort_column_id(self.TAG_DATA, gtk.SORT_ASCENDING)
