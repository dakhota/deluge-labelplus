...
from tagging.gtkui.common.widgets.autotag_box import AutotagBox
from tagging.gtkui.common.gtklib.radio_button_group import RadioButtonGroup
...
from tagging.common.config.autotag import PROPS

from tagging.common.config import (
  MOVE_PARENT, MOVE_SUBFOLDER, MOVE_FOLDER,
  TAG_DEFAULTS,
)

from tagging.common.literals import (
  TITLE_TAG_OPTIONS, TITLE_SELECT_FOLDER,
  STR_LOAD_OPTIONS, STR_SAVE_OPTIONS, STR_TAG, STR_PARENT, STR_NONE,
  ERR_TIMED_OUT, ERR_INVALID_TAG,
)
...
class TagOptionsDialog(WidgetEncapsulator):

  # Section: Constants

  GLADE_FILE = tagging.common.get_resource("wnd_tag_options.glade")
  ROOT_WIDGET = "wnd_tag_options"
  ...
  OP_MAP = {
    ...
    AutotagBox: ("set_all_row_values", "get_all_row_values"),
  ...
  def __init__(self, plugin, tag_id, page=0):

    self._plugin = plugin

    if tag_id in RESERVED_IDS or tag_id not in plugin.store:
    ...
    super(TagOptionsDialog, self).__init__(self.GLADE_FILE, self.ROOT_WIDGET,
      "_")

    try:
      self._store = plugin.store.copy()
      self._set_tag(ID_NULL)
  ...
  def destroy(self):
    ...
    if self.valid:
      ...
      super(TagOptionsDialog, self).destroy()
  ...
  def update_store(self, store):
    ...
    self._request_options(self._tag_id)


  # Section: General

  def _set_tag(self, tag_id):

    if tag_id in self._store:
      self._tag_id = tag_id
      self._tag_name = self._store[tag_id]["name"]
      self._tag_fullname = self._store[tag_id]["fullname"]
    else:
      self._tag_id = ID_NULL
      self._tag_name = _(STR_NONE)
      self._tag_fullname = _(STR_NONE)
  ...
  def _request_options(self, tag_id):

    def on_timeout():

      if self.valid:
        self._wnd_tag_options.set_sensitive(True)
        self._report_error(STR_LOAD_OPTIONS, TaggingError(ERR_TIMED_OUT))
    ...
    def process_result(result):
      ...
      if self.valid:
        ...
        for success, data in result:
          if not success:
            error = tagging.common.extract_error(data)
            if error:
        ...
        self._tag_defaults = result[0][1]
        self._move_path_options = result[1][1]
        self._tag_options = result[2][1]

        self._load_options(self._tag_options)
    ...
    if not tag_id in self._store:
      self._report_error(STR_LOAD_OPTIONS, TaggingError(ERR_INVALID_TAG))
      return

    self._set_error(None)

    log.info("Loading options for %r", self._tag_fullname)

    deferreds = []
    deferreds.append(client.tagging.get_tag_defaults())
    deferreds.append(client.tagging.get_move_path_options(tag_id))
    deferreds.append(client.tagging.get_tag_options(tag_id))

    deferred = twisted.internet.defer.DeferredList(deferreds,
      consumeErrors=True)

    self._wnd_tag_options.set_sensitive(False)
  ...
  def _save_options(self):
    ...
    def on_timeout(options):
      ...
      if self.valid:
        self._wnd_tag_options.set_sensitive(True)
        self._report_error(STR_SAVE_OPTIONS, TaggingError(ERR_TIMED_OUT))
    ...
    if not self._tag_id in self._store:
      self._report_error(STR_SAVE_OPTIONS, TaggingError(ERR_INVALID_TAG))
      return

    self._set_error(None)

    log.info("Saving options for %r", self._tag_fullname)

    options = self._get_options()
    same = tagging.common.dict_equals(options, self._tag_options)

    if self._chk_autotag_retroactive.get_active():
      apply_to_all = not self._chk_autotag_untagged_only.get_active()
    else:
      apply_to_all = None

    if not same or apply_to_all is not None:
      deferred = client.tagging.set_tag_options(self._tag_id, options,
        apply_to_all)
  ...
  def _setup_widgets(self):
    ...
    self._wnd_tag_options.set_transient_for(
      deluge.component.get("MainWindow").window)

    self._wnd_tag_options.set_title(_(TITLE_TAG_OPTIONS))
    icon = self._wnd_tag_options.render_icon(gtk.STOCK_PREFERENCES,
      gtk.ICON_SIZE_SMALL_TOOLBAR)
    self._wnd_tag_options.set_icon(icon)

    self._lbl_header.set_markup("<b>%s:</b>" % _(STR_TAG))
  ...
  def _setup_autotag_box(self):

    crbox = AutotagBox(row_spacing=6, column_spacing=3)
    if __debug__: RT.register(crbox, __name__)

    crbox.set_name("crbox_autotag_rules")
    self._crbox_autotag_rules = crbox
  ...
  def _setup_test_combo_box(self):
    ...
    prop_store = tagging.gtkui.common.gtklib.liststore_create(str,
      [_(x) for x in PROPS])
  ...
  def _setup_criteria_area(self):
    ...
    def on_mapped(widget, event):
      ...
      if self._plugin.config["common"]["tag_options_pane_pos"] > -1:
        self._vp_criteria_area.set_position(
          self._vp_criteria_area.allocation.height -
          self._plugin.config["common"]["tag_options_pane_pos"])
  ...
  def _index_widgets(self):
    ...
    self._option_groups = (
      ...
      (
        self._chk_autotag_settings,
        self._rb_autotag_match_all,
        self._crbox_autotag_rules,
      ),
    )
    ...
    self._dependency_widgets = {
      ...
      self._chk_autotag_settings: (self._blk_autotag_settings_group,),
      ...
      self._chk_autotag_retroactive: (self._chk_autotag_untagged_only,),
    }
  ...
  def _load_state(self):
    ...
    if self._plugin.initialized:
      pos = self._plugin.config["common"]["tag_options_pos"]
      if pos:
        self._wnd_tag_options.move(*pos)

      size = self._plugin.config["common"]["tag_options_size"]
      if size:
        self._wnd_tag_options.resize(*size)

      self._tgb_fullname.set_active(
        self._plugin.config["common"]["tag_options_fullname"])


  def _save_state(self):

    if self._plugin.initialized:
      self._plugin.config["common"]["tag_options_pos"] = \
        list(self._wnd_tag_options.get_position())

      self._plugin.config["common"]["tag_options_size"] = \
        list(self._wnd_tag_options.get_size())
  ...
  def _load_options(self, options):
    ...
    self._set_path_tag(path)


  def _get_options(self):

    options = copy.deepcopy(self._tag_defaults)
  ...
  def _revert_options_by_page(self, options_out, page):
    ...
    for widget in widgets:
      prefix, sep, name = widget.get_name().partition("_")
      if sep and name in self._tag_defaults:
        options_out[name] = copy.deepcopy(self._tag_defaults[name])
  ...
  def _clear_options(self):
    ...
    self._load_options(self._tag_defaults)
  ...
  def _refresh(self):
    ...
    if self._tag_id == ID_NULL:
      self._lbl_selected_tag.set_tooltip_text(None)
    else:
      self._lbl_selected_tag.set_tooltip_text(self._tag_fullname)
  ...
  def _do_toggle_fullname(self, *args):
    ...
    if self._tgb_fullname.get_active():
      self._lbl_selected_tag.set_text(self._tag_fullname)
    else:
      self._lbl_selected_tag.set_text(self._tag_name)
  ...
  def _on_txt_changed(self, widget):
    ...
    self._set_path_tag(widget.get_text())
  ...
  def _do_open_file_dialog(self, widget):
    ...
    dialog = gtk.FileChooserDialog(_(TITLE_SELECT_FOLDER),
      self._wnd_tag_options, gtk.FILE_CHOOSER_ACTION_SELECT_FOLDER,
      (
    ...
    widgets = tagging.gtkui.common.gtklib.widget_get_descendents(dialog,
      (gtk.ToggleButton,), 1)
    if widgets:
  ...
  def _do_test_criteria(self, *args):
    ...
    rules = self._crbox_autotag_rules.get_all_row_values()
    match_all = self._rb_autotag_match_all.get_active()

    log.debug("Properties: %r, Rules: %r, Match all: %s", props, rules,
      match_all)

    result = tagging.common.config.autotag.find_match(props, rules,
      match_all)
  ...
  def _create_menu(self):
    ...
    items = tagging.gtkui.common.gtklib.menu_add_items(self._menu, 0,
      (
