...
from tagging.common.tag import RESERVED_IDS


class TagSelectionMenu(gtk.Menu):
  ...
  def __init__(self, model, on_activate=None, headers=True, root_items=[],
    ...
    super(TagSelectionMenu, self).__init__()

    self.has_user_tags = False
  ...
  def _build_menu(self, model, on_activate, headers, root_items, sub_items):
    ...
    children = tagging.gtkui.common.gtklib.treemodel_get_children(model)
    ...
    if children:
      self.has_user_tags = True

    if root_items:
      self._items += tagging.gtkui.common.gtklib.menu_add_items(self, -1,
        root_items)
      if children:
        self._items.append(tagging.gtkui.common.gtklib.menu_add_separator(
          self))
  ...
  def _create_item(self, model, iter_, menu, on_activate, headers, sub_items):
    ...
    if not children and not sub_items:
      ...
      if headers:
        self._items += tagging.gtkui.common.gtklib.menu_add_items(submenu,
