...
from tagging.gtkui.common.gtklib.criteria_box import CriteriaBox

from tagging.common import _
from tagging.gtkui import RT


from tagging.common.config.autotag import (
  PROPS, OPS, CASES,
  FIELD_PROP, FIELD_OP, FIELD_CASE, FIELD_QUERY,
)


class AutotagBox(CriteriaBox):

  def __init__(self, homogeneous=False, row_spacing=0, column_spacing=0):

    super(AutotagBox, self).__init__(homogeneous, row_spacing,
      column_spacing)

    prop_store = tagging.gtkui.common.gtklib.liststore_create(str,
      [_(x) for x in PROPS])
    op_store = tagging.gtkui.common.gtklib.liststore_create(str,
      [_(x) for x in OPS])
    case_store = tagging.gtkui.common.gtklib.liststore_create(str,
      [_(x) for x in CASES])
  ...
  def get_all_row_values(self):
    ...
    rows = super(AutotagBox, self).get_all_row_values()
  ...
  def set_all_row_values(self, rows):
    ...
    super(AutotagBox, self).set_all_row_values(converted_rows)
