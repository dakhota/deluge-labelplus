...
from tagging.common.literals import (ERR_EMPTY_TAG, ERR_INVALID_CHARS)
...
def is_ancestor(ancestor_id, tag_id):
  ...
  return ancestor_id != tag_id and tag_id.startswith(prefix)
...
def validate_name(tag_name):

  if not tag_name:
    raise TaggingError(ERR_EMPTY_TAG)

  if RE_INVALID_CHARS.search(tag_name):
    raise TaggingError(ERR_INVALID_CHARS)
