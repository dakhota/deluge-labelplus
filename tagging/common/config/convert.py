...
def process_spec(spec, dict_in):
  ...
  for src, dest in sorted(spec["map"].items(), key=lambda x: x[1].count("/")):
    mapped = tagging.common.get_path_mapped_dict(dict_in, src, dest,
      spec["deepcopy"], spec["strict"])
    tagging.common.update_dict(working_dict, mapped)
  ...
  tagging.common.update_dict(dict_out, working_dict)
